...
export function FolderConfigPanel({ folder, onQueueProcessing }: Props) {
  ...
  const activeResolutions = Array.from(selectedResolutions).sort((a, b) => a - b);

  const allPairs: Array<{ resolution: number; frameCount: number; config: ResFrameConfig }> = [];
  for (const res of activeResolutions) {
    const frames = selectedFrames[res] || new Set();
    for (const f of Array.from(frames).sort((a, b) => a - b)) {
      const key = `${res}_${f}`;
      if (configs[key]) {
        allPairs.push({ resolution: res, frameCount: f, config: configs[key] });
      }
    }
  }
                  ...
                  return (
                    ...
                    >
                      ...
                      <Select
                        ...
                        onValueChange={(v) =>
                          updateConfig(key, { frameSampling: v as "uniform" | "head" })
                        }
                      >
